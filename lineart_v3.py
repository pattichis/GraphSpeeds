...
class cuteGraph:
    ...
    def __init__(self):
        ...
        self.setStep(stepX=1, stepY=1, stepPixels=20)
    ...
    # Setup a square grid
    def setStep(self, stepX, stepY, stepPixels):
      self.stepX = stepX
      self.stepY = stepY
    ...
    def setX(self, minX, maxX):
      ...
      """
      width = (maxX - minX + self.stepX) / self.stepX * self.stepPixels
      if (width <= 2048):
        self.minX  = minX
        self.maxX  = maxX
        self.width = width
    ...
    def setY(self, minY, maxY):
      ...
      height = (maxY - minY) / self.stepY * self.stepPixels
      if (height <= 2048):
        self.minY  = minY
        self.maxY  = maxY
        self.height = height
    ...
    def setupGrid(self, xdigits=3, xdecimals=0, ydigits=3, ydecimals=0, grid_color='green'):
      ...
      minX = self.minX
      maxX = self.maxX
      numSamples = int((maxX-minX)/self.stepX)+1
      ...
      minY = self.minY
      maxY = self.maxY
      numSamples = int((maxY-minY)/self.stepY)+1
...
def plotTablesLines(tables = None, 
    ...
    # Set the axes
    tmp.setStep(stepX=stepX, stepY=stepY, stepPixels=stepPixels)
